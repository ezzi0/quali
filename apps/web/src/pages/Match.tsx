...
const budgetMap: Record<string, { min: number; max: number | null }> = {
  'under-1m': { min: 0, max: 1000000 },
  '1m-1.5m': { min: 1000000, max: 1500000 },
  '1.5m-2.5m': { min: 1500000, max: 2500000 },
  '2.5m-5m': { min: 2500000, max: 5000000 },
  '5m-plus': { min: 5000000, max: null },
};
...
export default function Match() {
  ...
  const handleSubmit = async (data: Partial<MatchData>) => {
    ...
    try {
      ...
      const budgetConfig = data.budget ? budgetMap[data.budget] : undefined;

      await api.leads.createPublic({
        form_type: 'match',
        email: isEmail ? contactDetails : null,
        phone: !isEmail ? contactDetails : null,
        city: data.countryTimezone || null,
        budget_min: budgetConfig?.min ?? null,
        budget_max: budgetConfig?.max ?? null,
        move_in_date: data.handoverPreference || null,
        preferences: [
          data.downPayment,
          data.installmentPreference,
          data.strategy,
          data.riskStyle,
          data.exitPreference,
          data.contactPreference,
        ].filter(Boolean),
        message: `Budget: ${data.budget || 'n/a'} | Strategy: ${data.strategy || 'n/a'} | Handover: ${data.handoverPreference || 'n/a'}`,
        raw_payload: data,
      });
